...
UA = "FoundryWorks-MigrationCapture/1.0 (+read-only source preservation)"
TIMEOUT = 40
VIDEO_HOSTS = {
...
def first_party(url):
    ...
    return host in {"jeffreycarldmd.com", "www.jeffreycarldmd.com"}
...
def discover_urls():
    ...
    root_url = urljoin(SOURCE, "/sitemap.xml")
    root = get(root_url)
...
def template_family(url, title, h1s, soup):
    ...
    text = " ".join([title, *h1s]).lower()
    if path in ("", "/") or re.search(r"/dentist-albany-or-home-p\d+\.asp$", path):
        return "home"
    if any(x in path for x in ("request-appointment", "contact-us", "patient-forms")):
        return "conversion"
    if any(x in path for x in ("meet-dr-", "about-us", "tour-our-office")):
        return "about-team"
    if any(x in path for x in ("patient-info", "reviews", "enewsletter")):
        return "patient-resource"
    if re.search(r"/p/dentist-(?!albany-or-)[a-z-]+-p\d+\.asp$", path):
        return "location"
    ...
    for tag, attr in attrs:
        for el in soup.find_all(tag):
            ...
            if not raw or raw.startswith(("data:", "javascript:", "mailto:", "tel:", "#")):
                continue
            candidates = [part.strip().split()[0] for part in raw.split(",")] if attr == "srcset" else [raw]
